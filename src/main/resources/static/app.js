...
const wpmTag = document.querySelector('.gross span');
const netWpmTag = document.querySelector('.net span');
...
function initTyping(){
    ...
    if(characterIndex < characters.length - 1 && timeLeft > 0){
        ...
        cpmTag.innerText = characterIndex + errors;//cpm will not count errors
         console.log("paragraph size : ", paragraphs.length);
         console.log("characterIndex size : ", characterIndex);
         console.log("paragraph size : ", paragraphs.length);
	
           let wpm = Math.round((((characterIndex + errors) / 5) / ((maxTime - timeLeft)/60)  ) );
		
	       let netwpm = Math.round((((characterIndex - errors) / 5) / ((maxTime - timeLeft)/60)  ) );

        wpm = wpm < 0 || !wpm || wpm === Infinity ? 0 : wpm;  
        netwpm = netwpm < 0 || !netwpm || netwpm === Infinity ? 0 : netwpm;
        
         wpmTag.innerText = wpm;
        netWpmTag.innerText = netwpm;
    }
    else{
        inputField.value = "";
        clearInterval(timer);
